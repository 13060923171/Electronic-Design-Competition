...
void LCD_printc(unsigned char x, unsigned char y, unsigned char c_dat)
{
	...
	LCD_char_pos(x, y);
	LCD_write_dat(0x00);						//补空, 1bytes
	for(i = 0; i < 5; i++) LCD_write_dat(Font_code[c_dat - 32][i]);
	LCD_write_dat(0x00);   LCD_write_dat(0x00);	//补空, 2bytes
}
...
{
	unsigned char i;
	y = y * 6 + 1;					//最左边和最右边各空一字节
	LCD_write_dat_pos(x, y, 0x00);	//前面补一个空字节
	for(i = 0; i < 5; i++) LCD_write_dat_pos(x, y + i + 1, Font_code[c_dat - 32][i]);
}
...
/*void LCD_printn(unsigned char x, unsigned char y, unsigned int num, unsigned char num_bit, unsigned char num_font)
{
	...
	for(i = 0; i < num_bit; i++)
	if(num_font)LCD_printc(x, y + i, ' ');		//清显示区域
	else		LCD_printcs(x, y + i, ' ');		//清显示区域
	for(i = ii; i >= 0; i--)
	if(num_font)LCD_printc(x, y++, dat[i]);		//输出数值
	else		LCD_printcs(x, y++, dat[i]);	//输出数值
}
/*--------------------------------------------------------------*/
#else 					//(8*16字体)
/*--------------------------------------------------------------*/ 
